...
class Backup
{
    ...
    public function fileList()
    {
        ...
        foreach ($glob as $name => $file) {
            if (preg_match('/^\\d{8,8}-\\d{6,6}-\\d+\\.sql(?:\\.gz)?$/', $name)) {
                ...
                $part = $name[6];
                $fileName = "{$name[0]}{$name[1]}{$name[2]}-{$name[3]}{$name[4]}{$name[5]}";
                ...
                $info['compress'] = $extension === 'SQL' ? 0 : 1;
                $info['name'] = $fileName;
                $info['time'] = strtotime("{$name[0]}-{$name[1]}-{$name[2]} {$name[3]}:{$name[4]}:{$name[5]}");
                $list[$fileName] = $info;
            }
        }
        return $list;
    ...
    public function import($compress=0)
    {
        //还原数据
        $db = self::connect();
